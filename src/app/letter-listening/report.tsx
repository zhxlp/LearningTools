...
// 过去 3 个月的日期范围与全部日期列表（纯函数，提到模块作用域：不参与组件闭包，
// 稳定引用使 loadQuizRecords 可安全 useCallback，避免挂载 effect 反复重载）。
...
export default function Report() {
  ...
  // 组件挂载后载入记录：setState 放在 effect 内的异步回调里执行，
  // 避免 effect 体内同步 setState 触发级联渲染。
  useEffect(() => {
    (async () => {
      try {
