...
import { Volume2, Settings, ChartNoAxesColumn } from "lucide-react-native";
...
export default function LetterListening() {
  ...
  const [previousLetter, setPreviousLetter] = useState<ILetterInfo | null>(
    null,
  );
  const [options, setOptions] = useState<string[]>([]);
  const [gameCompleted, setGameCompleted] = useState(false);
  const [selectedIncorrect, setSelectedIncorrect] = useState<string | null>(
    null,
  );
  const [optionCount, setOptionCount] = useState<number>(5);
  const [letterCaseOption, setLetterCaseOption] =
    useState<LetterCaseOption>("uppercase");
  const [sessionCorrect, setSessionCorrect] = useState(0);
  const [sessionIncorrect, setSessionIncorrect] = useState(0);
  const [streakCount, setStreakCount] = useState(0);
  const wrongPlayer = useAudioPlayer(audioWrong);
  const feedbackPlayer = useAudioPlayer(audioA);
  const [wrongFeedbackMode, setWrongFeedbackMode] =
    useState<WrongFeedbackMode>("error");
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      (async () => {
        try {
          const [savedOptionCount, savedLetterCaseOption, savedWrongFeedback] =
            await Promise.all([
          ...
          if (
            savedWrongFeedback === "error" ||
            savedWrongFeedback === "letter"
          ) {
  ...
  const startNewGame = (count: number = optionCount) => {
    ...
    if (previousLetter) {
      availableLetters = LettersData.filter(
        (l) => l.letter !== previousLetter.letter,
      );
    }
    ...
    const incorrectOptions = shuffled
      ...
      .map((l) => l.letter);
    const allOptions = [selectedLetter.letter, ...incorrectOptions].sort(
      () => 0.5 - Math.random(),
    );
  ...
  // 每次渲染后刷新稳定引用（ref 写入需在 effect 内，渲染期写 ref 会被 React hooks 规则拦下），
  // 确保聚焦回调调用的是最新一版的 startNewGame。
  useEffect(() => {
  ...
  return (
    ...
    >
      <View style={styles.container}>
        ...
        >
          <ChartNoAxesColumn size={24} color="#1976d2" />
        </TouchableOpacity>

        {/* Statistics Display */}
        <View style={styles.statsContainer}>
          <Text style={styles.statsText}>本次答题: </Text>
          <Text style={[styles.statsText, styles.correctText]}>
            正确 {sessionCorrect}
          </Text>
          <Text style={[styles.statsText, styles.incorrectText]}>
            错误 {sessionIncorrect}
          </Text>
          <Text style={[styles.statsText, styles.streakText]}>
            连续 {streakCount}
          </Text>
        </View>
        ...
        {/* Audio Player */}
        {currentLetter ? (
          <AudioPlayer key={currentLetter.letter} audio={currentLetter.audio} />
        ) : null}
      ...
      <ParentalGateOverlay
        ...
        onSuccess={() => {
          ...
          router.push("/letter-listening/settings");
        }}
      />
    </SafeAreaView>
  );
