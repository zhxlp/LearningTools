...
// 载入近 3 个月轮次并按答题时间降序排列（纯数据获取，不做状态更新）。
async function fetchRecentRoundList(): Promise<MathRound[]> {
  const loaded = await loadRoundsSince(Date.now(), WINDOW_MS);
  loaded.sort(
    (a, b) =>
      new Date(b.firstAnswerAtISO).getTime() -
      new Date(a.firstAnswerAtISO).getTime(),
  );
  return loaded;
}
...
export default function MathTestReportScreen() {
  ...
  const [replay, setReplay] = useState<{
    attempt: MathAttempt;
    q: MathQuestionRecord;
  } | null>(null);
  // 重载触发器：清除轮次后自增，驱动下方 effect 重新拉取。
  const [reloadTick, setReloadTick] = useState(0);

  // 近 3 个月总体聚合（空窗口也返回全 0 的 byOp）。
  const agg = useMemo(() => aggregateRounds(rounds), [rounds]);

  // 载入近 3 个月轮次并排序：setState 放在 effect 内的异步回调里，避免同步 setState 级联渲染。
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const loaded = await fetchRecentRoundList();
        if (cancelled) return;
        setRounds(loaded);
  ...
  const handleClear = (): void => {
    Alert.alert("确认清除", "清除近 3 个月的轮次记录？此操作无法撤销。", [
      { text: "取消", style: "cancel" },
      {
        ...
        style: "destructive",
        onPress: () => {
          void deleteRoundsSince(Date.now(), WINDOW_MS).then(() =>
            setReloadTick((t) => t + 1),
          );
        },
      },
          ...
          <View style={styles.statsGrid}>
            <View style={styles.statItem}>
              <Text style={styles.statValue}>
                {ready ? agg.totalCorrect : "—"}
              </Text>
              <Text style={styles.statLabel}>正确</Text>
            </View>
            <View style={styles.statItem}>
              <Text style={styles.statValue}>
                {ready ? agg.totalIncorrect : "—"}
              </Text>
            ...
            <View style={styles.statItem}>
              <Text style={styles.statValue}>
                {ready
                  ? `${accuracy(agg.totalCorrect, agg.totalIncorrect)}%`
                  : "—"}
              </Text>
          ...
          {ALL_OPS.map((op) => {
            ...
            return (
              <View key={op} style={styles.tableRow}>
                <Text style={[styles.tableCell, styles.cellOp]}>
                  {OP_SYMBOLS[op]}
                </Text>
                ...
                <Text style={[styles.tableCell, styles.cellNum]}>
                  {accuracy(st.correct, st.incorrect)}%
                </Text>
              </View>
            );
          })}
              ...
              return (
                <View key={round.id}>
                  <TouchableOpacity
                    style={[
                      styles.recordItem,
                      expanded && styles.selectedRecord,
                    ]}
                    onPress={() => toggleRound(round.id)}
                    accessibilityRole="button"
                  >
                    <Text style={styles.recordTime}>
                      {fmtTime(round.firstAnswerAtISO)}
                    </Text>
                  ...
                  {expanded && (
                    <View style={styles.detailBlock}>
                      <View style={styles.detailStatsRow}>
                        ...
                        <View style={styles.statBox}>
                          <Text style={styles.statBoxValue}>
                            {accuracy(round.correct, round.incorrect)}%
                          </Text>
                          <Text style={styles.statBoxLabel}>准确率</Text>
                        </View>
                      </View>
                      <View style={styles.tableHeader}>
                        <Text style={[styles.tableHeaderCell, styles.cellExpr]}>
                          题目
                        </Text>
                        <Text style={[styles.tableHeaderCell, styles.cellNum]}>
                          正确
                        </Text>
                        <Text style={[styles.tableHeaderCell, styles.cellNum]}>
                          错误
                      ...
                      {round.questions.length === 0 ? (
                        ...
                        round.questions.map((q, index) => {
                          ...
                          const attempts = rawAttempts.length
                            ? [...rawAttempts].sort(
                                (x, y) =>
                                  new Date(y.atISO).getTime() -
                                  new Date(x.atISO).getTime(),
                              )
                            : [];
                          ...
                          const cells = (
                            <>
                              <Text
                                style={[styles.tableCell, styles.cellExpr]}
                                numberOfLines={1}
                              >
                                {q.expr}
                              </Text>
