...
// 还原方向锁：非“主动锁定”值（UNKNOWN/OTHER/读取失败）一律解锁回系统默认（跟随设备），
// 其余按原锁恢复。
function restoreOrientationLock(lock: ScreenOrientation.OrientationLock | null) {
  if (
    lock == null ||
    lock === ScreenOrientation.OrientationLock.UNKNOWN ||
    lock === ScreenOrientation.OrientationLock.OTHER
  ) {
...
export default function ParentalGateOverlay(
  props: ParentalGateOverlayProps,
): React.JSX.Element {
  const { visible, onSuccess, onClose } = props;

  const { width: winWidth, height: winHeight } = useWindowDimensions();
  // 窗口尺寸镜像进 ref：排版默认/载入只需在打开盖层那一刻取值，不因尺寸变化重开盖层。
  // ref 写入放在独立 effect 而非渲染期（渲染期写 ref 会被 React hooks 规则拦下）；
  // 声明在加载 effect 之前，每次提交先同步尺寸，加载时读到的是打开那一刻的值。
  ...
  const rightWidth =
    winWidth < 700 ? Math.max(190, Math.round(winWidth * 0.5)) : 296;
  ...
  // prop 变化时调整 state（React 文档认可的 "adjusting state when a prop changes" 模式）：
  // 打开盖层即清空上次输入/错误态/占位，避免在 effect 体内同步 setState 触发级联渲染。
  ...
  // 每次打开盖层时重新加载设置并生成新题，并同步主界面最新竖式排版。
  useEffect(() => {
    if (!visible) return;
    let cancelled = false;
  ...
  // 打开盖层即临时锁定横屏（竖屏来源如首页设置入口也会旋转过来），关闭时还原为
  // 打开前宿主设定的方向锁：首页未锁定→解锁回到跟随设备；听力/计算页锁定横屏→
  // 恢复锁横屏。不能按“来源是否横屏”猜，否则会把横屏锁残留到跟随设备的页面，
  // 导致盖层关掉后转不回竖屏。
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // 读失败按“未锁定”处理：关闭时回到系统默认方向（跟随设备）。
      }
      if (!active) {
        // 读取尚未完成就已被关闭：按刚读到的锁还原。
        restoreOrientationLock(prevLock);
        return;
      }
      prevLockRef.current = prevLock;
    ...
    return () => {
      active = false;
      restoreOrientationLock(prevLockRef.current);
    };
  }, [visible]);
