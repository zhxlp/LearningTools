...
export default function BoardReviewModal(
  props: BoardReviewModalProps,
): React.JSX.Element {
  ...
  const scaledColumnarStyle = useMemo<ColumnarStyle>(() => {
    const cs = attempt.columnarStyle ?? {
      digitSize: 44,
      rowGap: 20,
      colGap: 6,
      padTop: 20,
    ...
    return {
      ...
      // 旧提交快照可能没有 padTop：缺失按 20（当时的硬编码默认）。
      padTop: (cs.padTop ?? 20) * scale,
    };
      ...
      <View style={styles.screen}>
        ...
        <View style={styles.captionWrap}>
          <Text style={styles.caption}>
            {expr} = ?　提交{" "}
            <Text style={styles.captionSubmitted}>{attempt.submitted}</Text>{" "}
            <Text style={{ color: markColor, fontWeight: "700" }}>{mark}</Text>
            <Text style={styles.captionDivider}>
              　正确答案 {expectedResult}
            </Text>
          </Text>
        </View>
        ...
        <View style={styles.area} onLayout={onAreaLayout}>
          {cw <= 0 || ch <= 0 ? (
            ...
            <View style={[styles.boardBox, { width: boxW, height: boxH }]}>
              ...
              <View
                style={[
                  styles.columnarWrap,
                  {
                    width: boxW,
                    height: boxH,
                    // 与答题现场一致：竖式层相对画布顶部偏移 padTop；笔迹坐标
                    // 本就含该偏移，回放必须按同一偏移放置，否则竖式与手写错位。
                    paddingTop: scaledColumnarStyle.padTop,
                  },
                ]}
                pointerEvents="none"
              >
