...
import { MathQuestionRecord, MathRound, OpType } from "@/lib/math-types";
...
const round = (id: string, questions: MathQuestionRecord[] = []): MathRound => {
  const sums = questions.reduce(
    (acc, x) => ({
      correct: acc.correct + x.correct,
      incorrect: acc.incorrect + x.incorrect,
    }),
...
describe("loadRoundsSince", () => {
  test("returns only rounds within the window, skipping older and unrelated keys", async () => {
    ...
    await AsyncStorage.setItem(
      roundKey("1"),
      JSON.stringify(round("1", [q("+", 1, 0)])),
    );
    await AsyncStorage.setItem(
      roundKey("2"),
      JSON.stringify(round("2", [q("*", 3, 1)])),
    );
    await AsyncStorage.setItem(
      "unrelatedSettings",
