...
test("enabledOps strips unknown ops and de-duplicates", () => {
  expect(
    sanitizeParentalGateSettings({ enabledOps: ["+", "%", "-", "x", "+"] })
      .enabledOps,
  ).toEqual(["+", "-"]);
  expect(
    sanitizeParentalGateSettings({ enabledOps: ALL_OPS }).enabledOps,
  ).toEqual(ALL_OPS);
});
...
test("enabledOps empty/non-array/only-invalid falls back to default", () => {
  ...
  expect(
    sanitizeParentalGateSettings({ enabledOps: ["%", "x"] }).enabledOps,
  ).toEqual(["+", "-"]);
  expect(sanitizeParentalGateSettings({ enabledOps: "+-" }).enabledOps).toEqual(
    ["+", "-"],
  );
...
test("digits are clamped to 1..4", () => {
  expect(
    sanitizeParentalGateSettings({ digits: { min: 0, max: 9 } }).digits,
  ).toEqual({ min: 1, max: 4 });
  expect(
    sanitizeParentalGateSettings({ digits: { min: 5, max: 3 } }).digits,
  ).toEqual({ min: 3, max: 4 });
});

test("digits min>max is swapped after clamping", () => {
  expect(
    sanitizeParentalGateSettings({ digits: { min: 4, max: 2 } }).digits,
  ).toEqual({ min: 2, max: 4 });
  expect(
    sanitizeParentalGateSettings({ digits: { min: 9, max: 1 } }).digits,
  ).toEqual({ min: 1, max: 4 });
});

test("malformed digits fall back to default 4..4", () => {
  expect(sanitizeParentalGateSettings({ digits: "big" }).digits).toEqual({
    min: 4,
    max: 4,
  });
  expect(sanitizeParentalGateSettings({ digits: null }).digits).toEqual({
    min: 4,
    max: 4,
  });
  expect(sanitizeParentalGateSettings({ digits: { min: 3 } }).digits).toEqual({
    min: 4,
    max: 4,
  });
  expect(
    sanitizeParentalGateSettings({ digits: { min: "x", max: 2 } }).digits,
  ).toEqual({ min: 4, max: 4 });
});

test("wrongAnswerMode keeps only new|retry else default new", () => {
  expect(
    sanitizeParentalGateSettings({ wrongAnswerMode: "new" }).wrongAnswerMode,
  ).toBe("new");
  expect(
    sanitizeParentalGateSettings({ wrongAnswerMode: "retry" }).wrongAnswerMode,
  ).toBe("retry");
  expect(
    sanitizeParentalGateSettings({ wrongAnswerMode: "alwaysNew" })
      .wrongAnswerMode,
  ).toBe("new");
  expect(
    sanitizeParentalGateSettings({ wrongAnswerMode: 42 }).wrongAnswerMode,
  ).toBe("new");
});
